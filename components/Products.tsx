import { Droplets, FlaskConical, Check } from 'lucide-react'
...
const products = [
  ...
  },
  {
    ...
    ctaColor: 'bg-aqua hover:bg-aqua-light !text-navy',
  },
]
            ...
            return (
              <div key={p.name} className="border-2 border-border rounded-2xl overflow-hidden hover:border-sky hover:shadow-[0_16px_48px_rgba(21,101,192,0.1)] hover:-translate-y-1 transition-all duration-300">
                ...
                <div className={`bg-gradient-to-br ${p.gradient} p-10 text-center relative overflow-hidden flex flex-col items-center justify-center min-h-[180px]`}>
                  ...
                  {p.tag && (
                    <span className="inline-block mt-2 bg-aqua/25 text-aqua text-sm font-bold px-4 py-1 rounded-full relative z-10">{p.tag}</span>
                  )}
                </div>
                ...
                <div className="p-10">
                  <ul className="mb-7 divide-y divide-border">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-center gap-3 py-3 text-sm text-dark">
                        <div className="w-5 h-5 rounded-full bg-ice flex items-center justify-center flex-shrink-0">
